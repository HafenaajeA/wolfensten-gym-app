...
export default function ExerciseCard(props) {
  ...
  function handleSetIncrement() {
    // create a counter for individual to keep track of their doings//
  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4 className="text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-orange-600">
          0{i + 1}
        </h4>
        <h2 className="capitalize whitespace-nowrap truncate max-w-full text-lg sm:text-xl md:text-2xl flex-1 sm:text-center">
          {exercise.name.replaceAll("_", " ")}
        </h2>
        <p className="text-sm text-orange-600 capitalize">{exercise.type}</p>
      </div>
      <div className="flex flex-col">
        <h3 className="text-orange-600 text-sm">Muscle Groups</h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        ...
        <button
          ...
          className="flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-green-900 hover:border-orange-800 hover:text-orange-600 w-full duration-200"
        >
          <h3 className="hover:text-orange-600 text-sm capitalize">
            Sets completed
          </h3>
